...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    const response = await fetch(
      `${baseApiUrl.replace(/\/+$/, "")}/api/posts/${postId}/likes?page=${page}&limit=${limit}`,
      {
        method: "GET",
        headers: {
          accept: "*/*",
    ...
    if (!response.ok) {
      return toApiError("Failed to fetch post likes", response.status);
    }

    return toApiError("Invalid response from post likes service", 502);
  } catch {
    return toApiError("Unable to reach post likes service", 500);
  }
}
