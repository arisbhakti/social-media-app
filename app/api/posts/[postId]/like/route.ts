...
async function handleRequest(
  ...
  try {
    const response = await fetch(
      `${baseApiUrl.replace(/\/+$/, "")}/api/posts/${postId}/like`,
      {
        method,
        headers: {
          accept: "*/*",
    ...
    if (!response.ok) {
      return toApiError(
        method === "POST" ? "Failed to like post" : "Failed to unlike post",
        response.status
      );
    }

    return toApiError("Invalid response from post like service", 502);
  } catch {
    return toApiError("Unable to reach post like service", 500);
  }
}
