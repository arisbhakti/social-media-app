...
export async function GET(request: Request) {
  ...
  const limit = toPositiveInt(searchParams.get("limit"), 9);

  try {
    const response = await fetch(
      `${baseApiUrl.replace(/\/+$/, "")}/api/me/posts?page=${page}&limit=${limit}`,
      {
        method: "GET",
        headers: {
          accept: "*/*",
    ...
    if (!response.ok) {
      return toApiError("Failed to fetch my posts", response.status);
    }

    return toApiError("Invalid response from my posts service", 502);
  } catch {
    return toApiError("Unable to reach my posts service", 500);
  }
}
