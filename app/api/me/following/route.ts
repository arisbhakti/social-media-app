...
export async function GET(request: Request) {
  ...
  try {
    const response = await fetch(
      `${baseApiUrl.replace(/\/+$/, "")}/api/me/following?page=${page}&limit=${limit}`,
      {
        method: "GET",
        headers: {
          accept: "*/*",
    ...
    if (!response.ok) {
      return toApiError("Failed to fetch following users", response.status);
    }

    return toApiError("Invalid response from following users service", 502);
  } catch {
    return toApiError("Unable to reach following users service", 500);
  }
}
