...
export async function GET(request: Request) {
  ...
  try {
    const response = await fetch(
      `${baseApiUrl.replace(/\/+$/, "")}/api/me/followers?page=${page}&limit=${limit}`,
      {
        method: "GET",
        headers: {
          accept: "*/*",
    ...
    if (!response.ok) {
      return toApiError("Failed to fetch followers", response.status);
    }

    return toApiError("Invalid response from followers service", 502);
  } catch {
    return toApiError("Unable to reach followers service", 500);
  }
}
