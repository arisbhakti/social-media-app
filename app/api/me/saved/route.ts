...
export async function GET(request: Request) {
  ...
  try {
    const response = await fetch(
      `${baseApiUrl.replace(/\/+$/, "")}/api/me/saved?page=${page}&limit=${limit}`,
      {
        method: "GET",
        headers: {
          accept: "*/*",
    ...
    if (!response.ok) {
      return toApiError("Failed to fetch saved posts", response.status);
    }

    return toApiError("Invalid response from saved posts service", 502);
  } catch {
    return toApiError("Unable to reach saved posts service", 500);
  }
}
