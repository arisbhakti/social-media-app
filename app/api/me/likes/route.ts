...
export async function GET(request: Request) {
  ...
  try {
    const response = await fetch(
      `${baseApiUrl.replace(/\/+$/, "")}/api/me/likes?page=${page}&limit=${limit}`,
      {
        method: "GET",
        headers: {
          accept: "*/*",
    ...
    if (!response.ok) {
      return toApiError("Failed to fetch liked posts", response.status);
    }

    return toApiError("Invalid response from liked posts service", 502);
  } catch {
    return toApiError("Unable to reach liked posts service", 500);
  }
}
