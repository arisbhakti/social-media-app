...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    const response = await fetch(
      `${baseApiUrl.replace(/\/+$/, "")}/api/users/${encodeURIComponent(username)}/posts?page=${page}&limit=${limit}`,
      {
        method: "GET",
        headers: {
          accept: "*/*",
    ...
    if (!response.ok) {
      return toApiError("Failed to fetch user posts", response.status);
    }

    return toApiError("Invalid response from user posts service", 502);
  } catch {
    return toApiError("Unable to reach user posts service", 500);
  }
}
