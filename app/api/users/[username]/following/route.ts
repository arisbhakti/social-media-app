...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    const response = await fetch(
      `${baseApiUrl.replace(/\/+$/, "")}/api/users/${encodeURIComponent(username)}/following?page=${page}&limit=${limit}`,
      {
        method: "GET",
        headers,
        cache: "no-store",
    ...
    if (!response.ok) {
      return toApiError("Failed to fetch user following", response.status);
    }

    return toApiError("Invalid response from user following service", 502);
  } catch {
    return toApiError("Unable to reach user following service", 500);
  }
}
