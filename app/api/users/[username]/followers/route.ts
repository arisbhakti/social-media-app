...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    const response = await fetch(
      `${baseApiUrl.replace(/\/+$/, "")}/api/users/${encodeURIComponent(username)}/followers?page=${page}&limit=${limit}`,
      {
        method: "GET",
        headers,
        cache: "no-store",
    ...
    if (!response.ok) {
      return toApiError("Failed to fetch user followers", response.status);
    }

    return toApiError("Invalid response from user followers service", 502);
  } catch {
    return toApiError("Unable to reach user followers service", 500);
  }
}
